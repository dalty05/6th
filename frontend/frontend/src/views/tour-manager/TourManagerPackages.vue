...
const savePackage = async () => {
  ...
  try {
    ...
    if (editingPackage.value) {
      response = await axios.put(`/api/tour/admin/packages/${editingPackage.value.id}`, form.value)
    } else {
      response = await axios.post('/api/tour/admin/packages', form.value)
    }
...
const confirmDelete = async () => {
  try {
    await axios.delete(`/api/tour/admin/packages/${packageToDelete.value.id}`)
    await loadPackages()
