          ...
          <div class="top-partners-list">
            <div v-for="(partner, index) in topPartners" :key="partner.id" class="partner-item">
              ...
              <div class="partner-stats">
                <span class="clicks">{{ formatNumber(partner.total_clicks || 0) }} clicks</span>
    ...
    <div class="modal-overlay" v-if="showDetailsModal" @click.self="closeDetailsModal">
      <div class="modal-container large">
        ...
        <div class="modal-body" v-if="selectedPartner">
          ...
          <div class="detail-section" v-if="selectedPartner.referral_links && selectedPartner.referral_links.length > 0">
      ...
      <div class="modal-container">
        ...
        <div class="modal-body">
          ...
          <div class="form-actions">
            ...
            <button @click="generateCSVExport" class="btn-primary" :disabled="exporting">
...
const loadAnalytics = async () => {
  loading.value = true
  try {
    // Step 1: Load all referral links
    const linksResponse = await referralService.getLinks()
    const links = Array.isArray(linksResponse) ? linksResponse : []
    
    // Step 2: Load partner stats from referral service
    // For each partner, get their stats
    const partnerStatsPromises = partners.value.map(async (partner) => {
      try {
        const stats = await referralService.getPartnerStats(partner.id)
        return {
          ...partner,
          link_count: stats.total_links || 0,
    ...
    const updatedPartners = await Promise.all(partnerStatsPromises)
...
const generateCSVExport = async () => {
  ...
  try {
    ...
    if (exportOptions.value.summary) {
      csvContent += 'Partner Analytics Report\n'
      csvContent += `Generated: ${new Date().toLocaleString()}\n`
      csvContent += `Date Range: ${exportDateRange.value.start} to ${exportDateRange.value.end}\n\n`
      
      csvContent += 'Metric,Value\n'
      csvContent += `Total Partners,${totalPartners.value}\n`
      csvContent += `Active Partners,${activePartners.value}\n`
      csvContent += `Total Links,${totalLinks.value}\n`
      csvContent += `Total Clicks,${totalClicks.value}\n`
      csvContent += `Unique Visitors,${uniqueVisitors.value}\n`
      csvContent += `Avg Clicks per Partner,${avgClicksPerPartner.value}\n\n`
    }
    
    // Partners
    if (exportOptions.value.partners) {
      csvContent += 'Partners\n'
      csvContent += 'Partner Name,Email,Referral Code,Links,Total Clicks,Unique Visitors,Joined\n'
    ...
    if (exportOptions.value.links) {
      ...
      csvContent += 'Link Name,Link Code,Partner,Clicks,Unique Clicks,Conversions,Status,Created\n'
    ...
    a.href = url
    a.download = `partner_analytics_${new Date().toISOString().split('T')[0]}.csv`
    a.click()
